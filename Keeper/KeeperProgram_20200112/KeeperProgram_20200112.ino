...
void loop() {
  ...
  if (digitalRead(StartSW) && (digitalRead(Angel1) == LOW)) {
    ...
    if (abs(gyro) <= 7) {
      ...
      if(sig == 0){ //ボールが0の時
        if(goal_y > 10){ //ゴールの前（？）に戻る
          m = goal_y / goal_x;
          z = atan(m) + PI;
          motorfunction(z, 5*abs(goal_x), -gyro);
        }else{
          motorfunction(0, 0, 0);  
        }
      }else{ 
      	if(y > 50 && goal_y > 10){ //ボールが遠い ゴールの前に戻る
          m = goal_y / goal_x;
          z = atan(-m) + PI;
          motorfunction(z, 5*abs(goal_x), -gyro);
      	}else{
        ...
        }else if(goal_y > 30){  //ゴールから遠い
          if(abs(x) < 7) {  //ボールが近い
            motorfunction(0, 0 ,0);
          }else if(0 < x){ //左にボールがある
            motorfunction(3*3.14/4, 3*abs(x), -gyro); //左下
          }else{ //右にボール
            motorfunction(5*3.14/4, 3*abs(x), -gyro); //右下
          }
        }else{ //いい感じのとこにいる
          if(abs(x) < 7) { 
            motorfunction(0, 100, -gyro); //突進
          }else if(0 < x){
            motorfunction(3.14/2, 3*abs(x), -gyro); //左
          }else{
            motorfunction(3*3.14/2, 3*abs(x), -gyro); //右
          }
      	}
        }  
      }
