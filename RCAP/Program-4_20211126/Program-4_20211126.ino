...
float az, ball_dist, wrap, straight;
float angle;
int pixel;
uint32_t color;
...
void keeper() {
  ...
  if (abs(gyro) <= 10) {
    ...
    power = abs(gyro) / 2;   //  モーターの速度をgyroにする
...
void attacker() {
  if (digitalRead(GoalSW)) { // GoalSWは攻める方向をスイッチに入れる, 相手ゴールの座標は機体中心
    goal_sig = b_sig;
    goal_x = bg_x;
    goal_y = -bg_y;
  } else {
  ...
  if (abs(gyro) < 15) {
    ...
    if (-5 <= y && y <= 30) { //ボールが前(0≦y≦30)にあるとき
      ...
      if(abs(x) < 4){
        ...
        if(ball_front <= 25){
          if(goal_sig == 0){
            ...
            if(goal_y <= 27 && abs(goal_x) < 17){
              ...
              motorfunction(z, abs(goal_x) + abs(goal_y)*2.5 + 15, -gyro);
            }
          }
        } else {
        ...
        straight = 0;
        z = atan2(x, y);
        motorfunction(z, sqrt(x * x + y * y) + 30, -gyro);
      }
    ...
    digitalWrite(LED_BUILTIN, HIGH);
    power = abs(gyro) / 2;   //  モーターの速度をgyroにする
    if (gyro > 0) {         // 時計回りに回りすぎているとき
      turnCCW(power);
    } else if (gyro < 0) {  // 反時計回りに回りすぎているとき
      turnCCW(-power);
    } else {
    }
  }
...
void intHandle() {  // Lineを踏んだらlineflagをセットして止まる。
  ...
  while (digitalRead(INT_29) == HIGH) {     // Lineセンサが反応している間は繰り返す
    if (digitalRead(LINE1D) == HIGH) {     // lineを踏んだセンサーを調べる
      back_Line1(power*2);        // Lineセンサと反対方向へ移動する
...
void back_Line1(int power) {             // Lineセンサ1が反応しなくなるまで後ろに進む
  ...
  while ((digitalRead(LINE1D) == HIGH) || (digitalRead(LINE5D) == HIGH) || (digitalRead(LINE3D) == HIGH)) {
    ...
    motorfunction(azimuth, power*2, 0);  // azimuthの方向に進ませる
  }
  digitalWrite(LED_R, LOW);  // LED_R消灯
  motorStop();
}
