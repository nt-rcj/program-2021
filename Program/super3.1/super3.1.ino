  ...
  if (digitalRead(StartSW) == LOW) { // STartSW == Lowでスタート
    ...
    if(abs(gyro) < 4){
      if(progress == 0){//段階0
        ...
        turn = 0;//回るところの定数初期化
        motorfunction(PI, 37, -gyro);
        if(y < -10){
          find = 1;// yが十分負であるとき、次のペットボトルを見つけたとする
        }
        if(abs(y) < 15 && find == 1){//ペットボトルの横に到達し、次のペットボトルを見つけていたら次の段階に進む
          progress = 3;
        }
        ...
        if(turn == 6){//6回目に突入したら次の段階に進む
          ...
          if(abs(x) < 10 && find == 1){//ペットボトルの横に到達し、次のペットボトルを見つけていたら次の段階に進む
            progress = 5;
          }
        ...
        quadrant = 0;//回るところの定数初期化
        turn = 0;//回るところの定数初期化
        motorfunction(PI/40, 37, -gyro);
        if(10 < y && y != 4096){
          find = 1;//yが十分正で、見つけている(y≠4096)の時、次のペットボトルを見つけたとする
        }
        if(abs(y) < 10 && find == 1){//ペットボトルの横に到達し、次のペットボトルを見つけていたら次の段階に進む
          progress = 7;
        }      
